...
function injectBuildTime(_isDev: boolean) {
  return {
    ...
    transformIndexHtml(html: string) {
       ...
       */
      if (process.env.VITE_USE_HISTORY_ROUTING === '0') {
        next = next.replace(
          '<meta charset="UTF-8" />',
...
function emitSitemap() {
  ...
  return {
    ...
    closeBundle() {
      if (process.env.VITE_USE_HISTORY_ROUTING === '0') return
...
export default defineConfig(({ command }) => ({
  ...
  define: {
    ...
    'import.meta.env.VITE_USE_HISTORY_ROUTING': JSON.stringify(
      process.env.VITE_USE_HISTORY_ROUTING ?? '1',
    ),
  },
  plugins: [
    ...
    VitePWA({
      ...
      manifest: {
        ...
        icons: (() => {
          const suffix =
            process.env.VITE_USE_HISTORY_ROUTING === '0' ? '' : '-syng'
          return [
            {
