...
import type { DoReMiGameOptions, PitchDetectionProvider } from './useDoReMiGame'
import {
  useDoReMiGame,
  GRACE_PERIOD_MS,
  MAX_CENTS_DEVIATION,
 ...
 * starts the actor in onMounted — so the composable must run inside a mounted
 * component. mountGame mirrors useSingFly.test.ts's setup helper. The pure FSM
 * graph is covered by doReMiMachine.test.ts; here we assert the
 * useDoReMiGame ↔ machine contract and the existing timing/scoring behavior. */
const wrappers: { unmount: () => void }[] = []

function mountGame(options: DoReMiGameOptions) {
  let api!: ReturnType<typeof useDoReMiGame>
  const wrapper = mount({
    setup() {
      api = useDoReMiGame(options)
      return () => h('div')
    },
  })
...
function simulateSingingNote(
  ...
  const midiNote = Math.round(frequencyToMidi(baseFreq))
